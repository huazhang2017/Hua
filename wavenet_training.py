...
class WavenetTrainer:
    ...
    def validate(self):
        ...
        for (x, target) in iter(self.dataloader):
            ...
            correct_loss = torch.sqrt(torch.mean((predictions.type(torch.float32)-target.type(torch.float32))**2, dim=0))
            correct_pred = torch.eq(target, predictions)
            accurate_classifications += torch.sum(correct_pred).data.item()
            accurate_loss += torch.sum(correct_loss).data.item()
        ...
        avg_accuracy_loss = accurate_loss / len(self.dataloader)
        self.dataset.train = True
        self.model.train()
        return avg_loss, avg_accuracy,avg_accuracy_loss
